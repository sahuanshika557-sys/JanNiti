/**
 * 60 FPS Fullscreen Cinematic Animation, Physics & Storytelling Engine for JanNiti AI
 * Full-viewport camera tracking, 3-layer parallax, living skeletal character rigging,
 * environmental weather, India map vector projections, particle dynamics, and seamless visual morphing.
 */
...
export class CinematicCanvasEngine {
  ...
  public sceneProgress: number = 0; // 0 to 1
  public selectedSimulationOption: 'A' | 'B' | 'C' = 'C';
  ...
  private initClusterParticles() {
    ...
    for (let i = 0; i < 127; i++) {
      ...
      this.clusterParticles.push({
        ...
        life: 1,
        maxLife: 1,
        targetX: 600 + (Math.random() - 0.5) * 40,
        targetY: 350 + (Math.random() - 0.5) * 40
      });
    }
  }
  ...
  public setSceneState(scene: number, progress: number) {
    this.currentScene = scene;
    this.sceneProgress = progress;
    this.updateCameraTargets(scene, progress);
  }

  public setSimulationOption(opt: 'A' | 'B' | 'C') {
    this.selectedSimulationOption = opt;
  }

  private updateCameraTargets(scene: number, p: number) {
    switch (scene) {
      case 1: // Scene 1: Dawn / Phone light in darkness (Slow cinematic push-in)
        this.camera.targetX = 600;
        this.camera.targetY = 350;
        this.camera.targetZoom = 1.35 + p * 0.45;
        break;
      case 2: // Scene 2: Rain & damaged road overview (Pull back to reveal full environment)
        this.camera.targetX = 460 + p * 80;
        this.camera.targetY = 330;
        this.camera.targetZoom = 1.22;
        break;
      case 3: // Scene 3: Waveform ribbon tracking shot (Continuous horizontal tracking)
        this.camera.targetX = 540 + p * 260;
        this.camera.targetY = 310 - p * 30;
        this.camera.targetZoom = 1.18 - p * 0.12;
        break;
      case 4: // Scene 4: Many diverse people (Full width multi-character tracking)
        this.camera.targetX = 600;
        this.camera.targetY = 320;
        this.camera.targetZoom = 0.96;
        break;
      case 5: // Scene 5: Zoom out to India constellation (Smooth aerial ascent)
        this.camera.targetX = 600;
        this.camera.targetY = 350;
        this.camera.targetZoom = 0.8 + (1 - p) * 0.2;
        break;
      case 6: // Scene 6: 127 particles fusion (Gravitational orbital push)
        this.camera.targetX = 600;
        this.camera.targetY = 350;
        this.camera.targetZoom = 1.05 + p * 0.28;
        break;
      case 7: // Scene 7: Deep inside AI Understanding (3D orbital roll)
        this.camera.targetX = 600 + Math.sin(p * Math.PI * 2) * 35;
        this.camera.targetY = 340 + Math.cos(p * Math.PI * 2) * 25;
        this.camera.targetZoom = 1.38;
        break;
      case 8: // Scene 8: Multilingual resonance
        this.camera.targetX = 600;
        this.camera.targetY = 330;
        this.camera.targetZoom = 1.15;
        break;
      case 9: // Scene 9: Layered isometric map
        this.camera.targetX = 600 + p * 35;
        this.camera.targetY = 330;
        this.camera.targetZoom = 0.96;
        break;
      case 10: // Scene 10: Hotspot emergence
        this.camera.targetX = 580;
        this.camera.targetY = 340;
        this.camera.targetZoom = 1.25 + p * 0.15;
        break;
      case 11: // Scene 11: Silent area equity sonar (Subterranean camera dive)
        this.camera.targetX = 640;
        this.camera.targetY = 375;
        this.camera.targetZoom = 1.42;
        break;
      case 12: // Scene 12: Policymaker workspace
        this.camera.targetX = 600;
        this.camera.targetY = 330;
        this.camera.targetZoom = 1.1;
        break;
      case 13: // Scene 13: Human review & sanction seal
        this.camera.targetX = 600;
        this.camera.targetY = 330;
        this.camera.targetZoom = 1.22;
        break;
      case 14: // Scene 14: What-if interactive simulation
        this.camera.targetX = 600 + (this.selectedSimulationOption === 'A' ? -60 : 60);
        this.camera.targetY = 310;
        this.camera.targetZoom = 1.15;
        break;
      case 15: // Scene 15: Return to original citizen on completed road (Exact matching shot)
        this.camera.targetX = 460 - p * 40;
        this.camera.targetY = 310;
        this.camera.targetZoom = 1.25;
        break;
      case 16: // Scene 16: Final India network loop & closing light
      default:
        this.camera.targetX = 600;
        this.camera.targetY = 350;
  ...
  private update() {
    ...
    if (this.currentScene === 6) {
      const clusterP = this.sceneProgress;
  ...
  private render() {
    ...
    // Camera Transform (Matrix Translation & Scale)
    const cx = width / 2;
    const cy = height / 2;
    ...
    // Draw Continuous Fullscreen Living World
    this.drawLivingWorld(width, height);

    this.ctx.restore();
  }

  private drawLivingWorld(width: number, height: number) {
    const t = this.time;
    const p = this.sceneProgress;
    const scene = this.currentScene;

    // ==========================================
    // 🌟 SCENE 1: THE PHONE LIGHT IN SILENCE
    // ==========================================
    if (scene === 1) {
      // Total Darkness with subtle warm gradient
      this.ctx.fillStyle = '#020617';
      this.ctx.fillRect(0, 0, 1200, 700);

      // Single warm pulsing light in the center
      const lightRadius = 18 + p * 90;
      ...
      if (p > 0.35) {
        ...
        this.ctx.beginPath();
        for (let x = 600; x <= 840; x += 10) {
          const waveY = 360 + Math.sin(t * 10 + x * 0.08) * (18 * (p - 0.35) * 2);
    ...
    // ==========================================
    // 🌟 FULLSCREEN 3-LAYER LIVING ENVIRONMENT (Scenes 2 to 4, 14 to 16)
    // ==========================================
    if (scene <= 4 || scene >= 14) {
      // 1. BACKGROUND SKY & CLOUDS
      const skyGrad = this.ctx.createLinearGradient(0, 0, 0, 440);
      if (scene === 2 || scene === 3) {
        skyGrad.addColorStop(0, '#020617'); // Monsoon overcast
        skyGrad.addColorStop(0.7, '#1e293b');
        skyGrad.addColorStop(1, '#334155');
      } else if (scene >= 15) {
        skyGrad.addColorStop(0, '#0284c7'); // Bright clear sky
        skyGrad.addColorStop(0.7, '#38bdf8');
        skyGrad.addColorStop(1, '#fef08a'); // Warm sunny horizon
      } else {
    ...
    // ==========================================
    // 🌟 SCENE 2 & 3: CITIZEN IN RAIN & VOICE TRACKING
    // ==========================================
    if (scene === 2 || scene === 3) {
      // Pothole with water ripples
      this.ctx.fillStyle = '#090d16';
      this.ctx.beginPath();
      this.ctx.ellipse(540, 405, 80, 30, 0, 0, Math.PI * 2);
      this.ctx.fill();
      ...
      this.ctx.beginPath();
      this.ctx.ellipse(540, 405, 68 + Math.sin(t * 3) * 8, 24 + Math.cos(t * 3) * 4, 0, 0, Math.PI * 2);
      this.ctx.stroke();

      // Auto-Rickshaw slowing down and splashing
      const vehicleX = 750 - Math.min(210, p * 280);
      this.drawAutoRickshaw(vehicleX, 410, t);

      // The Primary Citizen (Ramesh in blue kurta)
      const citizenX = 410 + (scene === 2 ? 30 : 30 + p * 35);
      this.drawCitizenCharacter(citizenX, 385, t, true);

      // Physical Glowing Voice Ribbon leaving mouth
      this.drawVoiceWaveformStream(citizenX + 16, 350, t, scene, p);
    ...
    // ==========================================
    // 🌟 SCENE 4: MULTIPLE CITIZENS & INTERTWINED VOICE TRAILS
    // ==========================================
    if (scene === 4) {
      const citizens = [
      ...
      for (let i = 0; i < citizens.length; i++) {
        ...
        // Luminous Voice Ribbons intertwining and ascending into the sky
    ...
    // ==========================================
    // 🌟 SCENE 5 & 6: INDIA MAP REVEAL & 127 FUSION
    // ==========================================
    if (scene === 5 || scene === 6) {
      this.ctx.fillStyle = '#030712';
      this.ctx.fillRect(0, 0, 1200, 700);

      this.drawIndiaMapNetwork(600, 350, t, scene === 6 ? 0.3 : 1.0);
      ...
      if (scene === 6) {
        ...
        // Central Radiant Cluster Singularity
        const coreSize = 32 + p * 38;
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText('CLUSTER CL-1042: 127 VOICES', 600, 355);
      }
    }

    // ==========================================
    // 🌟 SCENE 7 & 8: DEEP INSIDE AI UNDERSTANDING
    // ==========================================
    if (scene === 7 || scene === 8) {
      this.ctx.fillStyle = '#020617';
      this.ctx.fillRect(0, 0, 1200, 700);

      this.drawFloatingDataMatrix(600, 340, t, p, scene);
    }

    // ==========================================
    // 🌟 SCENE 9 & 10 & 11: LAYERED MAP & EQUITY SONAR
    // ==========================================
    if (scene === 9 || scene === 10 || scene === 11) {
      this.ctx.fillStyle = '#050b14';
      this.ctx.fillRect(0, 0, 1200, 700);

      this.drawLayeredMapStack(600, 340, t, p, scene);
    ...
    // ==========================================
    // 🌟 SCENE 12 & 13: POLICYMAKER WORKSPACE & HUMAN SEAL
    // ==========================================
    if (scene === 12 || scene === 13) {
      this.ctx.fillStyle = '#090d16';
      this.ctx.fillRect(0, 0, 1200, 700);

      this.drawPolicymakerInterface(600, 330, t, p, scene);
    }

    // ==========================================
    // 🌟 SCENE 14: WHAT-IF INTERACTIVE SIMULATION
    // ==========================================
    if (scene === 14) {
      this.ctx.fillStyle = '#030712';
      this.ctx.fillRect(0, 0, 1200, 700);
    ...
    // ==========================================
    // 🌟 SCENE 15: RETURN TO EXACT SAME CITIZEN ON DRY ROAD
    // ==========================================
    if (scene === 15) {
      ...
      // Original smiling citizen walks smoothly
      ...
      this.ctx.lineWidth = 3.5;
      this.ctx.beginPath();
      this.ctx.arc(citizenX + 16, 350, 15 + Math.sin(t * 4) * 4, 0, Math.PI * 2);
      this.ctx.stroke();
    }

    // ==========================================
    // 🌟 SCENE 16: FINAL INFINITE LOOP & CLOSING LIGHT
    // ==========================================
    if (scene === 16) {
      this.ctx.fillStyle = '#020617';
      this.ctx.fillRect(0, 0, 1200, 700);
    ...
    // ==========================================
    // 🌟 FOREGROUND LAYER (Parallax Depth: Tree Silhouettes)
    // ==========================================
    if (scene <= 4 || scene >= 14) {
  ...
  private drawForegroundFoliage(x: number, y: number, time: number) {
    ...
    this.ctx.fillStyle = 'rgba(15, 23, 42, 0.75)'; // Dark foreground silhouette
    this.ctx.beginPath();
    this.ctx.moveTo(0, 0);
  ...
  private drawCitizenCharacter(x: number, y: number, time: number, isSpeaking: boolean, isHappy: boolean = false) {
    ...
    this.ctx.beginPath();
    this.ctx.arc(x + 10, y - 56 + breathe, 9, 0, Math.PI * 2);
    this.ctx.fill();
  ...
  private drawVoiceWaveformStream(x: number, y: number, time: number, scene: number, p: number) {
    this.ctx.save();
    this.ctx.strokeStyle = '#38bdf8';
    this.ctx.lineWidth = 3.5;
    this.ctx.shadowColor = '#0284c7';
    ...
    this.ctx.beginPath();
    const endX = scene === 3 ? x + 220 + p * 320 : x + 170;
  ...
  private drawFloatingDataMatrix(cx: number, cy: number, time: number, p: number, scene: number) {
    const layers = [
  ...
  private drawLayeredMapStack(cx: number, cy: number, time: number, p: number, scene: number) {
  ...
  private drawPolicymakerInterface(cx: number, cy: number, time: number, p: number, scene: number) {
    this.ctx.fillStyle = '#0f172a';
    this.ctx.strokeStyle = '#38bdf8';
    this.ctx.lineWidth = 2;
    ...
    const btnColor = scene === 13 && p > 0.5 ? '#10b981' : '#6366f1';
    ...
    this.ctx.font = 'bold 13px Inter, sans-serif';
    this.ctx.textAlign = 'center';
    this.ctx.fillText(scene === 13 && p > 0.5 ? '✓ SANCTIONED' : 'Review Evidence', cx - 140, cy + 55);
  ...
  private drawWhatIfSimulationDualRoad(cx: number, cy: number, time: number, p: number) {
    // Option A
    this.ctx.fillStyle = this.selectedSimulationOption === 'A' ? 'rgba(30, 41, 59, 0.95)' : 'rgba(15, 23, 42, 0.7)';
    this.ctx.strokeStyle = this.selectedSimulationOption === 'A' ? '#f43f5e' : '#475569';
    this.ctx.lineWidth = this.selectedSimulationOption === 'A' ? 3 : 1;
    ...
    // Option C (Integrated)
    this.ctx.fillStyle = this.selectedSimulationOption === 'C' ? 'rgba(30, 41, 59, 0.95)' : 'rgba(15, 23, 42, 0.7)';
    this.ctx.strokeStyle = this.selectedSimulationOption === 'C' ? '#10b981' : '#475569';
    this.ctx.lineWidth = this.selectedSimulationOption === 'C' ? 3 : 1;
